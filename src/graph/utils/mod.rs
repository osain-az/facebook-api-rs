...
impl FileResult {
    ...
    pub fn chunk_file(mut self, start: f64, current_blob_size: f64) -> FileResult {
        ...
        // The chunked size of the video to be uploaded  was initially supposed to be determined by the sized of the
        //  uploaded video but there are some issue in send ing the request with the form data when large chunk blob file is append to the FormDat
        // untill this fixed, a pre-defined chunked size of 5mb will be used  as default.

        let test_chunk_size = 5000000; // this will be the test chunked until the issue of large blob in the formdata is fixed.
        ...
        if file_size < half_gb_to_byte {
            ...
            // if the while is less the greater than 0.5 but less than 1gb, splite the process into 3 chunks
        ...
        let start = 0;
        let chunk_end = min(file_size, (start + chunk_size) as f64); // this is the ending chunk*/
...
//TODO remove get for getters
    ...
    pub fn get_upload_chunking_size(self) -> u64 {
        self.chunk_upload_size
    }
    pub fn get_uploading_method(self) -> String {
        self.upload_method
    }

    pub fn get_file_size_byte_f64(self) -> f64 {
        self.file_size_byte
    }

    pub fn get_file_size_byte_string(self) -> String {
